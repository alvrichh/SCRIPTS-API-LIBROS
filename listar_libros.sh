...
# URL del endpoint para listar libros
LISTAR_LIBROS_URL="http://localhost:8080/api/v1/libros"
...
# Realiza la solicitud GET al endpoint de listar libros usando el token JWT
curl -v -X GET -H "Authorization: Bearer $token" $LISTAR_LIBROS_URL
